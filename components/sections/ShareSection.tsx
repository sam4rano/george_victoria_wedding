...
export function ShareSection() {
  ...
  const handleNativeShare = useCallback(async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: COUPLE, text, url });
      } catch (err) {
  ...
  const handlePhotoChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !file.type.startsWith("image/") || file.size > MAX_PHOTO_SIZE_MB * 1024 * 1024) return;
    if (photoPreview) URL.revokeObjectURL(photoPreview);
  ...
  const captureCard = useCallback(async (pixelRatio: number = 2): Promise<string | null> => {
    ...
    try {
      return await toPng(node, { pixelRatio, backgroundColor: "oklch(0.98 0.012 85)", cacheBust: true });
    } finally {
  ...
  const showSuccessFeedback = useCallback(() => {
    ...
    confetti({ particleCount: 80, spread: 60, origin: { y: 0.7 }, colors: ["#8b4a5c", "#c9a9a6", "#f5f0eb"] });
  }, []);
